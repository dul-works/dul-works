...
import { extractText, extractDate, findProperty, extractRelation, findTitleProperty, extractNumber, getImageUrl, getBlockImage } from './notion-utils.js';
...
function extractImagesFromImageField(imageText) {
  ...
  const images = filenames.map(filename => {
    ...
    return {
      ...
      path: getImageUrl(normalizedFilename),
 ...
 */
async function extractPageText(blocks) {
  try {
...
export async function getExhibitionBySlug(slug, isBasicOnly = false) {
  try {
    ...
    // 텍스트 추출
    let pageText = await extractPageText(blocks);
    ...
    if (!isBasicOnly) {
      ...
      // 해당 전시의 작품 목록 가져오기
      artworks = await getArtworksByExhibition(matchingItem.id, detail.name);
    }
