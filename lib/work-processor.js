...
import { extractText, extractNumber, extractDate, findProperty, extractCheckbox, byIndexNullsLast } from './notion-utils.js';
import { getPageBlocksServer, getBlockChildrenServer, getDatabaseMetadataServer, getContainingPageId } from './notion-api-server.js';

/**
 * Work 항목에서 프로젝트 데이터 추출
 */
function extractProjectData(item) {
  ...
  /* Post Checkbox 추출 */
  const postProperty = findProperty(
    properties,
    'Post', 'post', 'POST'
  );
  const isPost = extractCheckbox(postProperty);
...
export function processWorkData(workData) {
  const projectItems = workData
    ...
    .filter(item => item.post === true);

  // Index 오름차순, Index 없는 항목은 맨 뒤로
  return projectItems.sort(byIndexNullsLast);
}
...
export async function getArtistStatement(workData) {
  try {
    ...
    const parentPageId = workDbMeta && await getContainingPageId(workDbMeta.parent);
    if (!parentPageId) {
      return null;
    }
